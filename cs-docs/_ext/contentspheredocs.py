"""
Sphinx plugins for contentsphere documentation.
"""
import json
import os
import re
...
def setup(app):
    ...
    app.add_description_unit(
        directivename="contentsphere-admin",
        rolename="djadmin",
        indextemplate="pair: %s; contentsphere-admin command",
        parse_node=parse_contentsphere_admin_node,
    )
    app.add_directive('contentsphere-admin-option', Cmdoption)
    app.add_config_value('contentsphere_next_version', '0.0', True)
    app.add_directive('versionadded', VersionDirective)
    app.add_directive('versionchanged', VersionDirective)
    app.add_builder(contentsphereStandaloneHTMLBuilder)
    ...
    app.set_translator('contentspherehtml', contentsphereHTMLTranslator)
    app.set_translator('json', contentsphereHTMLTranslator)
...
class VersionDirective(Directive):
    ...
    def run(self):
        ...
        if self.arguments[0] == env.config.contentsphere_next_version:
...
class contentsphereHTMLTranslator(HTMLTranslator):
    """
    contentsphere-specific reST to HTML tweaks.
    """
    ...
    #
    version_text = {
        'versionchanged': 'Changed in contentsphere %s',
        'versionadded': 'New in contentsphere %s',
    }
...
def parse_contentsphere_admin_node(env, sig, signode):
    command = sig.split(' ')[0]
    env.ref_context['std:program'] = command
    title = "contentsphere-admin %s" % sig
    signode += addnodes.desc_name(title, title)
    return command


class contentsphereStandaloneHTMLBuilder(StandaloneHTMLBuilder):
    """
    Subclass to add some extra things we need.
    """

    name = 'contentspherehtml'
    ...
    def finish(self):
        ...
        with open(outfilename, 'w') as fp:
            fp.write('var contentsphere_template_builtins = ')
